...
type AdmaruAdapter struct {
	URI string
}

// Builder builds a new instance of the Admaru adapter for the given bidder with the given config.
func Builder(bidderName openrtb_ext.BidderName, config config.Adapter, server config.Server) (adapters.Bidder, error) {
	bidder := &AdmaruAdapter{
		URI: config.Endpoint,
	}
	return bidder, nil
}

func (a *AdmaruAdapter) MakeBids(internalRequest *openrtb2.BidRequest, externalRequest *adapters.RequestData, response *adapters.ResponseData) (*adapters.BidderResponse, []error) {
...
func (a *AdmaruAdapter) MakeRequests(request *openrtb2.BidRequest, reqInfo *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {
...
type ExtBidderAdmaru struct {
	Bidder openrtb_ext.ExtImpAdmaru `json:"bidder"`
}
